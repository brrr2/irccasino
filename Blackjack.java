...
public class Blackjack extends CardGame {
	public static class RespawnTask extends TimerTask {
		...
		public void run() {
			...
			game.bot.sendMessage(game.channel, player.getNickStr()
					+ " has been loaned $" + game.getNewCash()
	...
	public static class HouseStat {
		...
		public String toString() {
			return numRounds + " round(s) have been played using " + numDecks
					+ " deck shoes. The house has won $" + cash
		...
		if (msg.charAt(0) == '.') {
			...
			if (msg.equals(".join") || msg.equals(".j")) {
				...
				if (channel.isOp(user)) {
					...
					bot.sendNotice(user,
							"Debugging commands may only be used by ops.");
				}
			} else if (msg.equals(".shuffle")) {
				if (isInProgress()) {
					bot.sendNotice(user,
							"A round is already in progress! Wait for the round to end.");
	...
	public void showPlayerTurn(Player p) {
		if (isBetting()) {
			bot.sendMessage(channel,
					p.getNickStr() + "'s turn. Stack: $" + p.getCash()
							+ ". Enter an initial bet up to $" + p.getCash()
							+ ".");
		} else {
	...
	public void showPlayerHandWithBet(BlackjackPlayer p, Hand h, int handIndex) {
		if (p.isDealer() || shoe.getNumberDecks() == 1) {
			bot.sendMessage(channel, p.getNickStr() + "-" + handIndex + ": "
					+ h.toString(1) + ", bet: $" + h.getBet());
		} else {
			bot.sendMessage(channel, p.getNickStr() + "-" + handIndex + ": "
					+ h.toString(0) + ", bet: $" + h.getBet());
		}
	}
	...
	public void showProperBet(BlackjackPlayer p) {
		bot.sendMessage(channel, p.getNickStr() + " bets $" + p.getInitialBet()
				+ ". Stack: $" + p.getCash());
	}

	public void showDoubleDown(BlackjackPlayer p, Hand h) {
		bot.sendMessage(channel, p.getNickStr()
				+ " has doubled down! The bet is now $" + h.getBet()
				+ ". Stack: $" + p.getCash());
	}
	...
	public void showInsure(BlackjackPlayer p, Hand h) {
		bot.sendMessage(channel, p.getNickStr()
				+ " has made an insurance bet of $" + h.getInsureBet()
				+ ". Stack: $" + p.getCash());
	}
	...
	public void showPlayerResult(BlackjackPlayer p, Hand h) {
		...
		if (h.hasSurrendered()) {
			...
			outStr += " pushes and his/her $" + h.getBet()
					+ " bet is returned. (";
		} else {
			outStr += " loses. (";
		}
	...
	public void showPlayerInsuranceResult(BlackjackPlayer p, Hand h, int index) {
		...
		if (result == 1) {
			p.addCash(3 * h.getInsureBet());
			house.addCash(-3 * h.getInsureBet());
			outStr = p.getNickStr() + "-" + index + " wins $" + 3
					* h.getInsureBet() + ".";
		} else {
	...
	public void infoInsureBetTooHigh(BlackjackPlayer p) {
		bot.sendNotice(p.getUser(),
				"Maximum insurance bet is $" + calcHalf(p.getInitialBet())
						+ ". Try again.");
	}
	...
	public void infoGameRules(User user) {
		if (playerJoined(user)) {
			...
			if (p.isSimple()) {
				bot.sendNotice(
						user,
						"Dealer stands on soft 17. The dealer's shoe has "
								+ shoe.getNumberDecks()
								+ " decks of cards. Cards are shuffled when the shoe is depleted.");
				bot.sendNotice(
						user,
						"Regular wins are paid out at 1:1 and blackjacks are paid out at 3:2. Insurance wins are paid out at 2:1");
			} else {
				bot.sendMessage(
						user,
						"Dealer stands on soft 17. The dealer's shoe has "
								+ shoe.getNumberDecks()
								+ " decks of cards. Cards are reshuffled when the shoe is depleted.");
				bot.sendMessage(
						user,
						"Regular wins are paid out at 1:1 and blackjacks are paid out at 3:2. Insurance wins are paid out at 2:1");
			}
			...
			bot.sendMessage(
					user,
					"Regular wins are paid out at 1:1 and blackjacks are paid out at 3:2. Insurance wins are paid out at 2:1. Early surrender is offered.");
		}
	}
	...
	public void infoGameHelp(User user) {
		if (playerJoined(user)) {
			...
			if (p.isSimple()) {
				bot.sendNotice(
						user,
						"For help on how to play "
								+ getGameNameStr()
								+ ", please visit an online resource. For game commands, type .gcommands.");
			} else {
				bot.sendMessage(
						user,
						"For help on how to play "
			...
			bot.sendMessage(
					...
					"For help on how to play "
							+ getGameNameStr()
							+ ", please visit an online resource. For game commands, type .gcommands. For house rules, type .grules.");
		}
	}
