...
public class TexasPoker extends CardGame {
	 ...
	 * 
	 * @param parent
	 *            The bot that created an instance of this ListenerAdapter
	 * @param gameChannel
	 *            The IRC channel in which the game is to be run.
	 * @param eb
	 *            The ListenerAdapter that is listening for commands for this
	 *            game
	 */
	...
	public void processCommand(User user, String command, String[] params) {
		...
		if (command.equals("join") || command.equals("j")) {
			if (parentListener.bjgame != null
					&& (parentListener.bjgame.isJoined(nick) || parentListener.bjgame
							.isWaitlisted(nick))) {
				bot.sendNotice(user, "You're already joined in "
						+ parentListener.bjgame.getGameNameStr() + "!");
			} else if (parentListener.bjgame != null
					&& parentListener.bjgame.isBlacklisted(nick)) {
		...
		} else if (command.equals("c") || command.equals("ca")
				|| command.equals("call")) {
			if (isPlayerTurn(nick)) {
				call();
			}
		} else if (command.equals("x") || command.equals("ch")
			...
			if (!channel.isOp(user)) {
				...
				if (params.length > 0) {
					...
					while (it.hasNext()) {
						User u = it.next();
						if (u.getNick().toLowerCase()
								.equals(fNick.toLowerCase())) {
							// Check if fNick is joined in another game
							if (parentListener.bjgame != null
									&& (parentListener.bjgame.isJoined(fNick) || parentListener.bjgame
											.isWaitlisted(fNick))) {
								bot.sendNotice(
										user,
										u.getNick()
												+ " is already joined in "
												+ parentListener.bjgame
														.getGameNameStr() + "!");
							} else if (parentListener.bjgame != null
									&& parentListener.bjgame
											.isBlacklisted(fNick)) {
								bot.sendNotice(user, u.getNick()
										+ " is bankrupt and cannot join!");
							} else {
		...
		} else if (command.equals("fc") || command.equals("fca")
				|| command.equals("fcall")) {
			if (isForcePlayAllowed(user, nick)) {
				call();
			}
		} else if (command.equals("fx") || command.equals("fch")
				|| command.equals("fcheck")) {
			if (isForcePlayAllowed(user, nick)) {
				check();
			}
			...
			if (isOpCommandAllowed(user, nick)) {
				if (params.length > 0) {
					try {
						...
						// Discard and shuffle
						for (int ctr = 0; ctr < number; ctr++) {
							deck.addToDiscard(peeps.get(ctr).getHand()
									.getAllCards());
						}
	...
	public void continueRound() {
		...
		if (currentPlayer == topBettor || currentPlayer == firstPlayer) {
			...
			if (stage == 4) {
				...
				dealCommunity();
				/*
				 * Only show dealt community cards when there are more than 1
				 * non-folded player remaining.
				 */
	...
	public void endRound() {
		...
		if (getNumberJoined() > 1 && pots.size() > 0) {
			...
			/*
			 * Clean-up tasks 1. Increment the number of rounds played for
			 * player 2. Remove players who have gone bankrupt and set respawn
			 * timers 3. Remove players who have quit mid-round 4. Save player
			 * data 5. Reset the player
			 */
			for (int ctr = 0; ctr < getNumberJoined(); ctr++) {
				p = (PokerPlayer) getJoined(ctr);
				p.incrementRounds();
	...
	public void leave(String nick) {
		...
		if (isJoined(nick)) {
			...
			if (isInProgress()) {
				/*
				 * If still in the post-start waiting phase, then currentPlayer
				...
				if (currentPlayer == null) {
					...
					bot.sendNotice(p.getNick(),
							"You will be removed at the end of the round.");
					fold();
				} else {
					p.setQuit(true);
					bot.sendNotice(p.getNick(),
					...
					if (!p.hasFolded()) {
						...
						// force call on that remaining player (whose turn it
						// must be)
						if (getNumberNotFolded() == 1) {
							call();
						}
				...
				// Just remove the player from the joined list if no round in
				// progress
			} else {
				removeJoined(p);
			}
	 ...
	 * 
	 * @param p
	 *            the player whose hand is to be discarded
	 */
	...
	/**
	 * Processes a check command. Only allow a player to check when the
	 * currentBet is 0 or if the player has already committed the required
	 * amount to pot.
	 */
	public void check() {
		cancelIdleOutTask();
	...
	/**
	 * Processes a call command. A player's bet will be matched to the
	 * currentBet. If a player's stack is less than the currentBet, the player
	 * will move all-in.
	 */
	public void call() {
		cancelIdleOutTask();
	...
	/**
	 * Process a fold command. The folding player is removed from all pots.
	 */
	public void fold() {
		cancelIdleOutTask();
		PokerPlayer p = (PokerPlayer) currentPlayer;
	...
	/**
	 * Adds the bets during a round of betting to the pot. If no pot exists, a
	 * new one is created. Sidepots are created as necessary.
	 */
	public void addBetsToPot() {
		...
		while (currentBet != 0) {
			...
			if (currentPot == null) {
				...
				// contributed
				// any bets in the latest round of betting, thus requiring a new
				// pot
	...
	public void showGameStats() {
		...
		if (totalPlayers == 1) {
			bot.sendMessage(channel, formatNumber(totalPlayers)
					+ " player has played " + getGameNameStr() + ".");
		} else {
			bot.sendMessage(channel, formatNumber(totalPlayers)
					+ " players have played " + getGameNameStr() + ".");
		}
	}

	public void showTablePlayers() {
		PokerPlayer p;
		String msg = Colors.BOLD + getNumberJoined() + Colors.BOLD
				+ " players: ";
		String nickColor = "";
		for (int ctr = 0; ctr < getNumberJoined(); ctr++) {
			p = (PokerPlayer) getJoined(ctr);
	...
	public void showCommunityCards() {
		...
		for (int ctr = 0; ctr < pots.size(); ctr++) {
			if (currentPot != pots.get(ctr)) {
				str = Colors.YELLOW + ",01Pot #" + (ctr + 1) + ": "
						+ Colors.GREEN + ",01$"
						+ formatNumber(pots.get(ctr).getPot()) + Colors.NORMAL
						+ " ";
				msg.append(str);
			}
		}
		...
		str = Colors.YELLOW + ",01Pot #" + (pots.size() + 1) + ": "
				+ Colors.GREEN + ",01$" + formatNumber(currentPot.getPot())
				+ Colors.NORMAL + " ";
		msg.append(str);
	...
	public void showTurn(Player p) {
		...
		bot.sendMessage(channel, p.getNickStr() + "'s turn. " + p.getNick()
				+ " in for $" + formatNumber(pp.getBet()) + ". Stack: $"
				+ formatNumber(p.getCash() - pp.getBet()) + ". "
				+ "Current bet: " + Colors.BOLD + "$"
				+ formatNumber(currentBet) + Colors.BOLD);
	}

	public void showBet(PokerPlayer p) {
		bot.sendMessage(channel,
				p.getNickStr() + " bets $" + formatNumber(p.getBet())
						+ ". Stack: $" + formatNumber(p.getCash() - p.getBet()));
	}

	public void showRaise(PokerPlayer p) {
		bot.sendMessage(channel,
				p.getNickStr() + " has raised to $" + formatNumber(p.getBet())
						+ ". Stack: $" + formatNumber(p.getCash() - p.getBet()));
	}

	public void showAllIn(PokerPlayer p) {
		bot.sendMessage(channel,
				p.getNickStr() + " has gone all in! " + p.getNick()
						+ " in for $" + formatNumber(p.getBet()) + ". Stack: $"
						+ formatNumber(p.getCash() - p.getBet()));
	}

	public void showCall(PokerPlayer p) {
		bot.sendMessage(channel, p.getNickStr() + " has called. " + p.getNick()
				+ " in for $" + formatNumber(p.getBet()) + ". Stack: $"
				+ formatNumber(p.getCash() - p.getBet()));
	}
	...
	public void showCheck(PokerPlayer p) {
		bot.sendMessage(channel,
				p.getNickStr() + " has checked. " + p.getNick() + " in for $"
						+ formatNumber(p.getBet()) + ". Stack: $"
						+ formatNumber(p.getCash() - p.getBet()));
	}

	public void showFold(PokerPlayer p) {
		bot.sendMessage(channel, p.getNickStr() + " has folded. Stack: $"
				+ formatNumber(p.getCash() - p.getBet()));
	}

	public void showPlayerResult(PokerPlayer p) {
		bot.sendMessage(channel, p.getNickStr() + " (" + p.getHand() + ")"
				+ ": " + p.getPokerHand().getName() + ", " + p.getPokerHand());
	}
	...
	public void showResults() {
		...
		for (int ctr = 0; ctr < pots.size(); ctr++) {
			...
			for (int ctr2 = 0; ctr2 < winners; ctr2++) {
				...
				bot.sendMessage(
						channel,
						Colors.YELLOW + ",01 Pot #" + (ctr + 1) + ": "
								+ Colors.NORMAL + " " + p.getNickStr()
								+ " wins $"
								+ formatNumber(currentPot.getPot() / winners)
	...
	/**
	 * Informs a player of his hand. The information is sent by notice if simple
	 * is true and by message if simple is false.
	 * 
	 * @param p
	 *            the player
	 * @param h
	...
	public void infoBetTooLow(String nick) {
		bot.sendNotice(nick, "Bet too low. Current bet is $"
				+ formatNumber(currentBet) + ".");
	}

	public void infoRaiseTooLow(String nick) {
		bot.sendNotice(nick, "Minimum raise is $" + formatNumber(minRaise)
				+ ". Try again.");
	}

	public void infoNoChecking(String nick) {
		bot.sendNotice(nick, "Current bet is $" + formatNumber(currentBet)
	...
	public String getGameRulesStr() {
		return "This is no limit Texas Hold'em Poker. Blind bets are set at $"
				+ formatNumber(getMinBet()) + "/$"
				+ formatNumber(getMinBet() * 2)
